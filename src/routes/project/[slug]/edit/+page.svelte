<script lang="ts">
  ...
  // let zipFile = (document.getElementById("zip") as HTMLInputElement).files?[0]: File

  function upload(){
    // let uploaded = new JSZip()
    // uploaded.loadAsync(zipFile).then(zip => {
  ...
  <div class="min-h-screen w-full flex-col items-center md:flex-row md:items-start md:pt-20">
    ...
    <div class="text-center align-middle md:text-start">
      <div class="flex rounded-xl bg-stone-800 p-2 py-3 space-x-2">
        ...
        <input id="icon" type="file" class="hidden" accept=".zip" on:input={upload}/>
...
<Modal bind:this={newVersion}>
  ...
  <input
    ...
    id="title" />
    <p class="align-middle font-brand dark:text-new-white-200">Drag to upload a file.</p>
    ...
    <p class="dark:text-white font-brand">Your file will be automatically compressed to reduce file size.</p>
</Modal>