...
<main
  ...
  <div class="h-screen w-full flex-col md:flex-row md:items-start md:pt-5">
    <h1
      class="mt-8 pb-2 text-center font-brand text-3xl font-bold text-pearl-lusta-900 dark:text-white md:text-start md:text-4xl lg:text-5xl">
      Site Roles
    </h1>
    ...
    {#await loadStuff() then}
      <table class="w-full table-auto p-2 text-left font-brand">
        <tr class="bg-emerald-500 p-2">
          ...
          <th class="p-2 text-pearl-lusta-900 dark:text-white">Permissions</th>
        </tr>
        {#each rolesJson ?? [] as i}
