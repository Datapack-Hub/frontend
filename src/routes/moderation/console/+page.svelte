...
// @ts-nocheck
    ...
    onMount(() => {
        ...
        let inpu = document.getElementById("inpt")
        alert(inpu)
        inpu.value = "your mother"
        inpu.onkeypress = async function (e) {
            if (!e) e = window.event;
            var keyCode = e.code || e.key;
            if (keyCode == "Enter") {
                const el = document.getElementById("inpt");
                if (el.value != "") {
                    ...
                    el.value = "Loading...";
                    try {
                        await postAuthed(apiURL + "/moderation/console", {command: cmd}).then((response) => {
                            response.text().then((text) => {
                                if (response.status == 200) {
                                    cons.innerHTML = cons.innerHTML + `<pre class='resp'>${text}</pre>`;
                                    el.value = "";
                                } else {
                                    cons.innerHTML = cons.innerHTML + `<li class='pb-4 text-red-500'>${text}</li>`;
                                    el.value = "";
                                }
                            });
                        });
...
<main class="dark:bg-stone-900 bg-newWhite transition-all px-4">
    {#if $userData.role != "default" && $isAuthenticated == true}
        {#if !isSmallWidth}
            <div class="flex-col md:items-start w-full h-screen md:pt-20 pt-10 text-lime-400 font-console" >
                ...
                <div class="bottom-0 right-0 absolute bg-black p-3 w-full flex justify-around">
