  ...
  {#if $isAuthenticated}
    {#if ["moderator", "developer", "admin"].includes($roleInfo.name)}
      <a
        ...
        <IconShield height="24" width="24" color="{iconColor}" style={{color: iconColor}} />
      </a>
    {/if}
    <a href="/notifications" class="z-20" use:tippy="{notificationHoverMsg}">
      {#if notifsAvailable}
        <IconUnread height="24" width="24" color="{iconColor}" style={{color: iconColor}} />
      {:else}
        <IconRead height="24" width="24" color="{iconColor}" style={{color: iconColor}} />
      {/if}
    </a>
    <a
